...
bp = Blueprint('messages', __name__, url_prefix='/<workspace_id>/admin')
routes_logger = logging.getLogger('routes')
@bp.route('/messages')
@login_required
@admin_required
def admin_messages(workspace_id):
    try:
...
#
#     if response.status_code != 200:
#         return f"Failed to fetch email content: {response.text}", 400
#
...
#     else:
#         decoded_body = "(No readable content found)"
#
#     return f"<h2>Email Content</h2><pre>{decoded_body}</pre>"