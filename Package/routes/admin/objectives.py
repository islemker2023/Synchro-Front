...
def admin_objectives(workspace_id):
    ...
    try:
        ...
        objectives = db_session.query(Objective).filter(
            ...
            joinedload(Objective.creator)  # This should load the Users object directly
        ...
        for objective in objectives:
            obj_data = {
                ...
                'created_at': objective.created_at.strftime('%B %d, %Y'),
                'rate': f"{objective.get_completion_percentage()}%",  # Need to implement this method
                'admin_name': objective.creator.full_name if objective.creator else "Unknown",  # Fixed
                'teacher_name': objective.teacher.user.full_name if objective.teacher and objective.teacher.user else "Unassigned",
                # Fixed
                'status': objective.get_status().lower().replace('_', '-'),  # Need to implement this method
                'due_date': objective.due_date.strftime('%B %d, %Y') if objective.due_date else "No due date",
                # Added null check
            }
...
def create_objective(workspace_id):
    ...
    try:
        ...
        assigned_to = request.form.get('assigned_by', '').strip()  # Keep form field name for compatibility
        ...
        teacher = db_session.query(Teachers).join(Users).filter(
            Users.full_name == assigned_to,
            Teachers.workspace_id == workspace_id
        ).first()

        if not teacher:
            # Alternative: try to find by email if that's what they're entering
            teacher = db_session.query(Teachers).join(Users).filter(  # Fixed: Added Teachers model
                Users.email == assigned_to,
                Teachers.workspace_id == workspace_id
            ).first()

            if not teacher:
                flash(f'Teacher "{assigned_to}" not found in this workspace', 'error')
                return redirect(url_for('admin.admin_objectives.admin_objectives', workspace_id=workspace_id))

        # Set priority with proper validation
        ...
        objective = Objective(
            ...
            created_by=UUID(current_user_id) if isinstance(current_user_id, str) else current_user_id,
            # Ensure it's a UUID
            title=title,
