...
bp = Blueprint('announcement', __name__, url_prefix='/<workspace_id>/teacher')
routes_logger = logging.getLogger('routes')

@bp.route('/announcement', methods=['GET', 'POST'])
@login_required
@teacher_required
def teacher_announcement(workspace_id):
    try:
        ...
        if request.method == 'POST' and form.validate_on_submit():
            ...
            try:
                ...
                return redirect(url_for('teacher.announcement.teacher_announcement',workspace_id=workspace_id))
...
@login_required
@teacher_required
def teacher_delete_announcement(notice_id,workspace_id):
    try:
    ...
    return redirect(url_for('teacher.announcement.teacher_announcement'))


@bp.route('/announcement/edit/<notice_id>', methods=['GET', 'POST'])
@login_required
@teacher_required
def teacher_edit_announcement(notice_id,workspace_id):
    # Get current user's workspace
    current_user_id = session.get('user_id')
    # Find the announcement
    notice = db_session.query(Notices).filter_by(
    ...
    return render_template('teacher/announcement.html',form=form, notice=notice)