...
bp = Blueprint('teacher_announcement', __name__, url_prefix='/routes/announcement')
...
def teacher_announcement():
    ...
    if not user_workspace:
        ...
        return redirect(url_for('teacher.teacher_announcement'))
...
@bp.route('/teacher/announcement/delete/<notice_id>', methods=['POST'])
@admin_required
@login_required
def teacher_delete_announcement(notice_id):
    try:
        # Get current user's workspace
        current_user_id = session.get('user_id')
        user_workspace = db_session.query(WorkspaceMember).filter_by(
...
@bp.route('/teacher/announcement/edit/<notice_id>', methods=['GET', 'POST'])
@admin_required
@login_required
def teacher_edit_announcement(notice_id):
    # Get current user's workspace
    current_user_id = session.get('user_id')
    user_workspace = db_session.query(WorkspaceMember).filter_by(
        user_id=current_user_id,
    ...
    return render_template('teacher/edit_announcement.html', notice=notice)